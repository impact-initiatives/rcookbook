...
3- When selecting options for select_multiple questions, ensure that the options are separated by a forward slash ("/").
...
```{r}
label2xml_question <- function(tool_survey, tool_choices, col){
  
  # for each column check if it is a select multiple
  if (str_detect(col, "/")) {
    q.label <- str_split(col, "/")[[1]][1]
    c.label <- paste0(tail(str_split(col, "/")[[1]], -1), collapse="/")
  } else {
    q.label <- col
    c.label <- NA
  }
  
  # returning the xml and make sure to include the xml of multiple choices after "/"
  if (q.label %in% tool_survey$`label::English`){
    q <- tool_survey[tool_survey$`label::English` %in% q.label, ]
    q.name <- q$name
    if (is.na(q.name) | q$type %in% c("note")) q.name <- q.label
    if (!is.na(c.label)){
      q.list_name = ifelse(q$list_name == "NA", NA, q$list_name)
      c.name <- tool_choices[tool_choices$list_name %in% q.list_name & tool_choices$`label::English` %in% c.label, "name"]
    } else c.name <- NA
    name <- ifelse(is.na(c.name), q.name, paste0(q.name, "/", c.name))
  } else name <- q.label
  return(name)
}
```
...
```{r}
label2xml_choices_one <- function(tool_survey, tool_choices, data, col) {
  
  # select the type column from each select_one question
  q.type <- tool_survey$type[tool_survey$`label::English` %in% col]
...
```{r}
label2xml_choices_multiple <- function(tool_survey, tool_choices, data, col) {
  ...
  for(j in 1:length(col_internal)){
    ...
    label_answer <- str_split(col_internal[j], "/")[[1]][2]
    d.join <- d.join %>% 
    ...
    choice_id <- tool_survey %>%
      filter(`label::English` %in% str_split(col_internal[j],"/")[[1]][1]) %>% 
      select(list_name)
    choice_id <- choice_id$list_name
    t.choices <- tool_choices %>% 
      filter(list_name == choice_id) %>% 
...
filename_labeled_data <- "enter your data hear"
...
col_select_one <- tool_select_one$`label::English`

for (i in 1:length(col_select_one)){
  if(!is.null(data_xml[[col_select_one[i]]])){
    data_xml[[col_select_one[i]]] <- label2xml_choices_one(tool_survey, tool_choices, data_labeled, col_select_one[i])
  }
}
...
tool_select_multiple <- tool_survey %>% 
  filter(str_starts(type, "select_multiple "))
col_select_multiple <- tool_select_multiple$`label::English`

for (i in 1:length(col_select_multiple)){
  if(!is.null(data_xml[[col_select_multiple[i]]])){
    data_xml[[col_select_multiple[i]]] <- label2xml_choices_multiple(tool_survey, tool_choices, data = data_labeled, col = col_select_multiple[i])
  }
}
...
for (i in 1:length(col_label)) {
  colnames(data_xml)[i] <- label2xml_question(tool_survey, tool_choices, col = col_label[i])
}
```
