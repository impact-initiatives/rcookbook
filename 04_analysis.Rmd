...
```{r impactR-load-data}
# Obtain data directly from the REACH Resource Center
library(rio)
...
## It means that unprotected water springs and open pit is the most common profile for rural hhs (12%), where as the most common profile for urban hhs is flush and pour toilet and tap water with national network (7%).

```

### Third stage: one analysis at a time with the Kobo tool

Now that we understands the `svy_*()` family of functions, we can use `make_analysis()` which is a wrapper around those for making analysis for medians, means, ratios, proportions of select ones and select multiples. The output has standardized columns,
...
Now we can use a dap, which mainly consists of a dataframe with needed informations for performing the analysis. One line is one requested analysis. All columns must be present. Usually an excel sheet is the easiest way to go as it can be co-constructed by both an AO and a DO.
