...
select_one_topX <- function(data, question_name, X=3){ # by default return top 3
  # test message
  if(length(colnames(data)[grepl(question_name, colnames(data), fixed=T)])==0){
...
shelter_type_top4 <- select_one_topX(data = main_dataset, question_name = "e1_shelter_type", X=4)
```
...
select_multiple_topX <- function(data, question_name, X=3){
  # test message
  if(length(colnames(data)[grepl(question_name, colnames(data), fixed=T)])==0){
    stop(print(paste("question name:", question_name,"doesn't exist in the main dataset. Please double check and make required changes!")))
  }
  ...
  df <- data %>%
    select(colnames(data)[grepl(paste0(question_name, "."), colnames(data), fixed=T)])
...
shelter_enclosure_issues_top7 <- select_multiple_topX(data = main_dataset, question_name = "e3_shelter_enclosure_issues", X=7)
```
