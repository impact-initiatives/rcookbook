...
(For example, it is possible to say that satisfaction with public transport and living in urban / rural areas are connected. However, we can conclude from the test itself, that "satisfied" and "dissatisfied" options differ, while "indifferent" does not. By the way, it is possible to come up with this conclusion while interpreting a corresponding crosstab. Nor chi-squared test can tell us about the strength and the direction of a liaison between variables).
...
```{r}
non_zero_for_test <- function(var, strata, data, per_cell = F){
sum <- as.data.frame(sapply(data[, names(data) %in% var], sum))
sum <- cbind(rownames(sum), sum)
filtered <- sum[sum[,2] > 0,1]
tab <- as.data.frame(sapply(main_dataset[,names(data) %in% filtered], table, main_dataset[, strata]))
...
```{r}
nz_f12 <- non_zero_for_test(f12, "strata", main_dataset)
...
```{r}
chi2_ind_wtd <- function(x, y, weight, data){
i <- 1
...
for (i in 1:length(x)) {
  ...
  chi_test  <- wtd.chi.sq(var1 = data[, paste0(y)], var2 = data[, paste0(ni)], weight = data[, paste0(weight)])
  ti$chisq <- chi_test[1]
  ti$df <- chi_test[2]
  ti$p <- chi_test[3]
