...
select_one_topX <- function(question_name = "e1_shelter_type" , X=3){ # by default return top 3
  ...
  if(nrow(df)==0) print("None of the choices was selected!")
 
  return(df)
}

shelter_type_top4 <- select_one_topX(question_name = "e1_shelter_type", X=4)
```
...
select_multiple_topX <- function(question_name, X=3){

  df <- main_dataset %>%
    select(colnames(main_dataset)[grepl(paste0(question_name, "."), colnames(main_dataset), fixed=T)])
  
  # if question was not answered
  if(all(is.na(df))) print("None of the choices was selected!")
...
shelter_enclosure_issues_top7 <- select_multiple_topX(question_name = "e3_shelter_enclosure_issues", X=7)
```
